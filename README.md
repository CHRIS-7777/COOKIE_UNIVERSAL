...
Single interactive CLI for \*\*decoding, verifying, brute-forcing and re-encoding\*\* all common web session cookies â€” `JWT`, `Flask/Django (itsdangerous)`, `Express`, `Generic HMAC`, `AES-CBC`, `Base64` â€” with \*\*auto SecLists wordlist selection\*\* and \*\*tamper loop\*\*.
...
/home/kali/TOOLS/sess\_enc\_dec/universal-cookie-tool.py
...
\- \*\*Only 2 options\*\* â†’ rest auto: `1. Decode` / `2. Encode`

\- \*\*Auto-detect\*\* algorithm from token format

\- \*\*Without secret\*\* â†’ shows `Header/Payload JSON` + `Algorithm name` (for tampering)

\- \*\*With secret\*\* â†’ `Valid âœ… / Invalid âŒ` (like jwt.io, with `BASE64URL` toggle)

\- \*\*Tamper loop\*\* â†’ decoded JSON shown pretty â†’ choose `Paste` or `Open in editor (nano/vim)` with arrow keys â†’ re-encode with same/new alg/secret

\- \*\*Auto brute-force\*\* â†’ auto-picks `SecLists` based on type (`scraped-JWT-secrets.txt` for JWT, `rockyou.txt` for Flask/Express/HMAC/AES) â€” just press Enter

\- \*\*Auto-fix JSON\*\* â†’ `'` â†’ `"` and `=` â†’ `:` (e.g. `{"id"=22}` â†’ `{"id":22}`)
...
`UNKNOWN` â†’ shown as raw + length, fallback tries JWT.
...
Paste Token/Session Cookie: .eJyrViotTi2Kz0xRsjI0NjbXAXPzEnNTlayUElNyM\_OUdJSK8nMQ3FoAnVoQMA:1x8Z6q:y7pkb6Qev3wZmuKpedLOSQ6Qqho

\[\*] Detected: FLASK | itsdangerous-compressed

\[Decoded Payload] {"user\_id":1337,"username":"admin","role":"admin"}
...
Enter Secret: mysecret -> Valid âœ… -> tamper value -> encode via Express
...
Enter Secret: secret123 + BASE64URL N -> Invalid âŒ -> Y for brute-force -> found N4AigW... -> tamper
...
\- \*\*Editor:\*\* choose `2` â†’ `nano` â†’ use arrows up/down, edit, `Ctrl+O` `Enter` `Ctrl+X` (vim: `:wq`)

\- \*\*Flask raw?\*\* Still JSON dict â€” edit same way. If raw string, shown as raw.
...
| `UNKNOWN / Base64 decode failed: Expecting value` with `:` token `.eJyr...:ts:sig` | Old version only handled `.` not `:` | \*\*Fixed\*\* â€” now `:` auto-converted to `.` and `zlib` decompressed. Update via `cp /home/kali/TOOLS/sess\_enc\_dec/universal-cookie-tool.py /home/kali/TOOLS/universal-cookie-tool.py` |

| `Invalid Signature` with `secret123` | Token not signed with that secret (`TyrFwe...` â‰  `N4AigW...`) | Use `Y` brute-force or paste correct token `...N4AigWa...` |
...
| Paste breaks | Used `=` not `:` | Auto-fixed now, but use `:` and `"` â€” e.g. `{"id":22}` |
